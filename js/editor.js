...
$(function () {
    ...
    }; // for removing model solutions and tests
...
/*
    $("#addGH").click(function () { // the code for the buttons for adding new elements
        if (gradingHintCounter === 1) {
            newGH();
        } // only one grading hint allowed
        $("#tabs").tabs("option", "active", tab_page.MAIN);
    }); // where this will be added
*/
    ...
    $("#submit_test").click(function () {
        ...
        let blob1 = new Blob([submissionXml], {
                type: 'application/xml'
            });
        formData.append('file', blob1, 'submission.xml');
        let blob2 = new Blob([taskXml], {
                type: 'application/xml'
            });
